...
<div class="section">
    <h6>Customer &amp; Proforma Information</h6>
    <div class="info-grid">
        <div><strong>Customer:</strong><span>{{ $proforma->customer->name ?? '-' }}</span></div>
        <div><strong>Proforma No:</strong><span>#{{ $proforma->row_no }}</span></div>
        <div><strong>Email:</strong><span>{{ $proforma->customer->email ?? '-' }}</span></div>
        <div><strong>Posted Date:</strong><span>{{ $proforma->posted_at }}</span></div>
        <div><strong>Phone:</strong><span>{{ $proforma->customer->phone ?? '-' }}</span></div>
        <div><strong>Expiry Date:</strong><span>{{ $proforma->expiry_at }}</span></div>
        <div><strong>Job:</strong><span>{{ $proforma->job_no }}</span></div>
        <div><strong>Reference No:</strong><span>{{ $proforma->reference_no ?? '-' }}</span></div>
        <div><strong>Currency:</strong><span>{{ $proforma->currency }} (rate {{ number_format($proforma->currency_rate, decimals()) }})</span></div>
        <div><strong>Status:</strong><span>{{ \App\Enums\ProformaInvoiceEnum::tryFrom($proforma->status)?->label() ?? '-' }}</span></div>
    </div>
</div>
...
<div class="section">
    ...
    @else
        <div class="text-center py-4 text-muted">No line items on this proforma invoice.</div>
    @endif
</div>
