<div>
@php
    $allSuppliers     = $supplierLedgerData['suppliers']          ?? [];
    $grandTotalDebit  = $supplierLedgerData['grand_total_debit']  ?? 0;
    $grandTotalCredit = $supplierLedgerData['grand_total_credit'] ?? 0;
    $netBalance       = $supplierLedgerData['net_balance']        ?? 0;

    // Filter to suppliers with activity
    $activeSuppliers = [];
    foreach ($allSuppliers as $id => $data) {
        if (abs($data['opening_balance'] ?? 0) > 0.001
            || count($data['transactions'] ?? []) > 0
            || abs($data['closing_balance'] ?? 0) > 0.001) {
            $activeSuppliers[$id] = $data;
        }
    }
...
@if(count($activeSuppliers) > 0)
    @foreach($activeSuppliers as $supplierId => $supplierData)
        @php
            $opening = (float)($supplierData['opening_balance'] ?? 0);
            $closing = (float)($supplierData['closing_balance'] ?? 0);
            $totalDr = (float)($supplierData['total_debit']    ?? 0);
            $totalCr = (float)($supplierData['total_credit']   ?? 0);
            $txns    = $supplierData['transactions'] ?? [];
        @endphp

        {{-- Supplier block: table --}}
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tr class="gl-acc-total">
                    <td colspan="5" class="ps-4 gl-acc-total-label">
                        <i class="bi bi-sigma me-1"></i>Supplier Total
                    </td>
...
@else
    <div class="text-center py-5 text-muted">
        ...
        <div class="x-small">Select a different date range or supplier to view transactions.</div>
    </div>
@endif
</div>

{{-- Bank-statement style layout: used for Print and PDF export only --}}
<div id="sl-print" class="stmt-print d-none d-print-block"
     data-pdf-filename="SupplierLedger-{{ $startDate ?? '' }}-{{ $endDate ?? '' }}.pdf">
    ...
    <table class="stmt-meta">
        <tr>
            ...
            <td class="text-end">
                <div class="stmt-title">SUPPLIER LEDGER</div>
                <div class="stmt-sub">Period: {{ \Carbon\Carbon::parse($startDate)->format('d M Y') }} — {{ \Carbon\Carbon::parse($endDate)->format('d M Y') }}</div>
                @if(count($activeSuppliers) > 0)
                    @php $printSupplier = reset($activeSuppliers); @endphp
                    <div class="stmt-sub">Supplier: {{ $printSupplier['supplier_code'] }} — {{ $printSupplier['supplier_name'] }}</div>
                @endif
                <div class="stmt-sub">Generated: {{ now()->format('d M Y H:i') }} &nbsp;|&nbsp; Currency: {{ optional(authUserCompany())->base_currency ?? 'SAR' }}</div>
            </td>
        </tr>
    </table>

    @forelse($activeSuppliers as $id => $sup)
        <table class="stmt-table">
            ...
            <tr class="stmt-strong">
                ...
                <td class="text-end">{{ number_format($sup['opening_balance'], 2) }}</td>
            </tr>
            ...
            <tr class="stmt-strong">
                ...
                <td class="text-end">{{ number_format($sup['total_debit'], 2) }}</td>
                <td class="text-end">{{ number_format($sup['total_credit'], 2) }}</td>
                <td class="text-end">{{ number_format($sup['closing_balance'], 2) }}</td>
            </tr>
            </tfoot>
        </table>
    @empty
...
<style>
/* ── Supplier Ledger table — provisional report design system ── */
