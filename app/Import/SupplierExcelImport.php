...
class SupplierExcelImport implements ToModel, WithStartRow
{
    protected $mapping;
    protected $errors = [];
    ...
    public function __construct(array $mapping)
    {
        ...
        $this->nextRowNo = (Supplier::max('unique_row_no') ?? 0);
    }
    ...
    public function model(array $row)
    {
        try {
            ...
            $data['row_no'] = 'SP' . sprintf("%03d", $this->nextRowNo);
            $data['company_id'] = companyId();
            $data['user_id'] = auth()->id();
            $data['currency'] = 'SAR';
            $data['status'] = SupplierStatusEnum::CONFIRMED->value;
            ...
            if (isset($data['name_en'])) {
                $rules['name_en'] = [
                    'required',
                    Rule::unique('suppliers', 'name_en')->where('company_id', $data['company_id'])
                ];
            }
            ...
            if (!empty($data['vat_number'])) {
                $rules['vat_number'] = [
                    'required',
                    Rule::unique('suppliers', 'vat_number')->where('company_id', $data['company_id'])
                ];
            }
            ...
            // IMPORTANT: Ensure these fields are in your Supplier model's $fillable array
