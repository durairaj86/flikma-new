...
class InputTax extends Component
{
    ...
    {
        $child = new InputTaxTable();
        $child->startDate = $this->startDate;
        $child->endDate = $this->endDate;
        $child->search = $this->search;
        $data = $child->getInputTaxData();

        $rows = [];
        foreach ($data['input_tax_transactions'] as $t) {
        ...
        $totalsRow = ['', '', '', 'TOTAL', (float) $data['total_input_tax']];
        ...
        $filename = 'InputTax-' . $this->startDate . '-' . $this->endDate . '.xlsx';
