AIRWAY_BILL = {
    ...
    filter: {
        load: function () {
            AIRWAY_BILL.filter.filterBox();
            AIRWAY_BILL.filter.searchBox();
        },
        filterBox: function () {
            $('#apply-filter').off().on({
                click: function () {
                    AIRWAY_BILL.list.dataTable();
    ...
    list: {
        ...
        dataTable(activeTab = null) {
            ...
            let table = $('#dataTable').DataTable({
                ...
                ajax: {
                    url: GLOBAL_FN.buildUrl('bl/airway-bill/data'),
                    type: 'POST',
                    data: function (d) {
                        d.tab = activeTab;
                        d.filterData = AIRWAY_BILL.filter.default();
                    },
                    ...
                    },
                    {
                        data: 'origin_destination', render: function (data, type, row) {
                            ...
                            if (row.origin_airport) {
                                origin_destination = '<span class="text-secondary small me-1">Origin:</span><span>' + row.origin_airport + '</span>';
                            }
        ...
        extraActions(row) {
            AIRWAY_BILL.list.actions.statusChange(row);
            AIRWAY_BILL.list.actions.view(row);
            AIRWAY_BILL.list.actions.delete(row);
        },
        actions: {
            statusChange(row) {
                $('#row_pending,#row_delivered,#row_cancelled,#row_in_transit').off().on('click', function () {
                    let fd = new FormData();
                    changeCustomerStatus(GLOBAL_FN.buildUrl('bl/airway-bill/' + row.attr('data-id') + '/status/' + $(this).attr('data-value')), {
                        method: 'POST',
                        data: fd,
                        callBack: 'datatable'
            ...
            print(row) {
                $('#row_print').off().on('click', function () {
                    let AIRWAY_BILLId = row.attr('data-id');
                    AIRWAY_BILL.printPreview(AIRWAY_BILLId);
                });
            }
        }
    },
    form: {
        ...
        open() {
            $('#new').off().on('click', function () {
                webModal.openGlobalModal({
                    title: 'New Airway Bill',
                    url: GLOBAL_FN.buildUrl('bl/airway-bill/create'),
                    content: {
        ...
        polPodLoad(preLoad = null) {
            ...
            initTomSelectSearch('#destination_airport', 'air', 100, preLoad);
            /*initTomSelectSearch('#carrier', port + 'Lines', 50, preLoad);*/
        },
        addRow() {
            $('#AIRWAY_BILL-tbody').off('click', '.add-row').on('click', '.add-row', function () {
        ...
        removeRow() {
            $('#AIRWAY_BILL-tbody').off('click', '.remove-row').on('click', '.remove-row', function () {
                let $tbody = $(this).closest('tbody');
