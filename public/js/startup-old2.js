...
CALCULATION = {
    ...
    finalTotals() {
        ...
        $('#' + MODULE + '-tbody tr').each(function () {
            const $row = $(this);
            let raw = $row.find('.row-total').val();
            raw = raw.replace(/,/g, '');
...
webDataTable = {
    ...
    `);
        ...
        default(row) {
            ...
            actions.edit(row);
            window[MODULE].list.extraActions(row);
        },
        edit(row) {
            $('#row_edit').off().on({
                click() {
        ...
        const ts = new TomSelect(el, {
            ...
            plugins: [
                ...
                onInitialize() {
                    ...
                    const maxAllowed = maxItemsAttr || null;
                    const ts = this;

                    // --- MULTIPLE SELECT WITH "X ITEMS SELECTED" MODE ---
                    // --- MULTIPLE SELECT WITH "X ITEMS SELECTED" MODE ---
                    if (isMultiple && maxAllowed) {

                        const ts = this;

                        // Update summary / show chips
                        const updateSummary = () => {
                            const chips = ts.control.querySelectorAll('.item');

                            if (ts.items.length > maxAllowed) {
                                // Show summary text
                                ts.control_input.placeholder = `${ts.items.length} items selected`;

                                // Keep items hidden but clickable for removal
                                chips.forEach(chip => {
                                    chip.style.opacity = 0;     // invisible
                                    chip.style.pointerEvents = 'none'; // avoid blocking click
