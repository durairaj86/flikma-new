...
// Fires when a Turbo page loads or is revisited
//commented
/*document.addEventListener("turbo:load", () => {
    // 1. Re-detect Module (Important as the body attributes change)
    MODULE = $("body").attr("data-module");
    MODULE = MODULE ? MODULE.toUpperCase() : "";

    // 2. Safely initialize page functions
    try {
        PAGE.init(); // Assuming PAGE.init() is safe

        // Use a timeout to ensure all external scripts (like DataTables)
        // have a moment to finish loading and defining their functions.
        setTimeout(function () {
            // Call GLOBAL_FN.load() which triggers DataTables and other setup
            GLOBAL_FN.load();

            // Execute module-specific load function safely
            if (MODULE && window[MODULE] && typeof window[MODULE].load === 'function') {
                // ⚠️ CRUCIAL: try...catch wrapper for module-specific code
                try {
                    window[MODULE].load();
                } catch (e) {
                    console.error(`⚠️ Module Load Error (${MODULE}):`, e);
                    // Allow script to continue to avoid breaking Turbo navigation.
                }
            }
        }, 100);
...
// Listener for Turbo before page cache/removal
//commented
/*document.addEventListener('turbo:before-cache', function () {
    // ⚠️ CRUCIAL: Wrap the destruction call in a try...catch
    try {
        GLOBAL_FN.destroyDataTable();
    } catch (e) {
        console.error("⚠️ DataTables Destruction Error (Cache):", e);
        // Continue, do not stop Turbo.
    }
});*/
...
/*window.addEventListener("error", function (event) {
    console.error("⚠️ Error Detected:", event.message);
    window.turboHasError = true;
}, true);

document.addEventListener("turbo:click", (event) => {
    if (window.turboHasError) {
        console.warn("🔁 Reloading due to previous error...");
...
let GLOBAL_FN = {
    ...
    activityFeed() {
        $('#activity-feed').click(function () {
            let drawer = new bootstrap.Offcanvas(document.getElementById('moduleFeedViewDrawer'));
            drawer.show();

            // Load Overview
            $('#feedOverview').html('<p>Loading...</p>');
            $.get('/log/activities/feed', function (data) {
                $('#feedOverview').html(data);
            });
        })
    },
    listNavTabs() {
...
CALCULATION = {
    ...
    finalTotals() {
        ...
        $('#' + MODULE + '-tbody tr').each(function () {
            const $row = $(this);
            let amount = $row.find('.row-total').val() || 0;

            amount = noComma(amount);
    ...
    submitForm(modal) {
        $('#globalModal').off('submit', 'form').on('submit', 'form', function (e) {
            ...
            if ($(this).valid()) {
                ...
                const fieldCount = form.find('.is-invalid').length;

                let consolidatedMessage = 'Validation Failed';
                consolidatedMessage += `: ${fieldCount} field${fieldCount > 1 ? 's' : ''} need to be fixed.`;

                toastr.error(consolidatedMessage);
            }
        });
    },
    buttons(modalId) {
    ...
    actions: {
        menu() {
            $('#dataTable').off('click', ".dropdown a.btn").on('click', ".dropdown a.btn", function (e) {
                ...
                $('#dataTable').find(".dropdown .dropdown-menu").not(dropdownDiv).removeClass("show").empty();
        ...
        edit(row) {
            $('#row_edit').off().on({
                click() {
                    ...
                    let title = dataTableData.data('title');
    ...
    $("input.datepicker").each(function () {
        ...
        flatpickr(this, {
            ...
            onChange: function (selectedDates, dateStr, instance) {
                // This manually triggers the Livewire update
...
function initTomSelectForm2222(form = null, destroy = false, individualId = null) {
    if (individualId) {
        form = $(individualId);
    } else {
...
function initTomSelectForm(form = null, destroy = false, individualId = null) {
    ...
    form.each(function () {
        ...
        const liveSearch = $el.data('live-search') === true || $el.data('live-search') === 'true';
        const create = $el.data('create') === true || $el.data('create') === 'true';
        const summaryLabel = $el.data('summary-label') ?? 'customers';
        const callBack = $el.data('call-back') ?? false;
        const callBackDisabled = $el.data('call-back-disabled') ?? false;
        ...
        const ts = new TomSelect(el, {
            ...
            onInitialize() {
                ...
                if (isMultiple) {
                    ...
                    this.updateSelectionSummary = () => {
                        ...
                        // remove old summary
                        const old = this.control.querySelector('.ts-summary');
                        if (old) old.remove();

                        // show all items back
                        this.control.querySelectorAll('.item').forEach(i => {
                            i.style.display = '';
                        });

                        if (count < SUMMARY_LIMIT) return;
                        ...
                        summary.className = 'item ts-summary';
                        summary.textContent = `${count} ${summaryLabel} selected`;

                        this.control.appendChild(summary);
                    };
                }
            ...
            render: {
                option(data, escape) {
                    ...
                    return `
                        <div class="option ts-option-item" data-value="${escape(data.value)}">
                            ${escape(data.text)}${sub}
                        </div>
                    `;
                },
            ...
            /*onItemAdd() {
                if (isMultiple && this.updateSelectionSummary) {
                    this.updateSelectionSummary();
                }
            },*/
            ...
            onItemAdd(value) {
                if (value === "__new__") {
                    const option = this.input.querySelector(`option[value="${value}"]`);
                    const module = option?.getAttribute('data-module');
                    ...
                    const option = this.input.querySelector(`option[value="${value}"]`);
                    const callBackValue = option?.getAttribute('data-call-value');
                    if (callBack === 'customerList') {
                        //callCustomerListLoadTomSelect(callBackValue);
                        callCustomerSelectTomSelect(callBackValue, callBackDisabled);
                    }
                }
...
function initTomSelectSearch(selector, dbName = 'sea', maxLength = 100, preLoad = null) {
    const $selector = $(selector);
    const placeholder = $selector.data('placeholder') ?? $selector.attr('placeholder') ?? '';
    const isMultiple = $selector.prop('multiple');
    new TomSelect(selector, {
        // ... (existing configuration) ...
        valueField: 'combined_value', // Using 'name' for the value
        labelField: 'name',
        create: false,
        placeholder: placeholder,
        maxOptions: 100,
        openOnFocus: true, // Keep this for when the user clicks the control manually
        maxItems: 1,
        allowEmptyOption: true,
                ...
                .then(json => {
                    const data = json
                        ...
                        .map(item => ({
                            ...
                            name: item.name,
                            code: item.code,
                            combined_value: `${item.code} - ${item.name}`
                        }));
                    callback(data);
                })
                .catch(() => callback());
        ...
        render: {
            option: function (data, escape) {
                ...
                const subtext = data.code
                    ? `<div class="ts-subtext">${escape(data.code)}</div>`
                    : '';

                // IMPORTANT: The data-value is correctly set to 'id' here,
                // but the selected VALUE of the component will be 'name'
                // due to valueField: 'name'. This might be intentional.
                return `
        <div class="option" data-selectable data-value="${escape(data.id)}">
            ${escape(data.name)}
            ${subtext}
        </div>
    `;
            },
            // The 'item' render function is highly recommended if 'option' is customized
            item: function (data, escape) {
                let dataValue = data.code ? data.code + ' - ' + data.name : data.name;
                return `<div class="item">${escape(dataValue)}</div>`;
            }
        },
        /*onDropdownClose() {
...
function callCustomerSelectTomSelect(customerId, callBackDisabled) {
    const control = document.getElementById('customer').tomselect;
...
function callCustomerListLoadTomSelect(id) {
    const loader = $('#customer-loader');

    // Initialize or get the Tom Select instance
    // Note: 'select#customer' should match your HTML
    let customerSelect = document.getElementById('customer').tomselect;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // 2. Map your response to the format Tom Select needs (value and text)
            const options = response.map(item => ({
                value: item.id,
                text: `${item.row_no} - ${item.name_en}` // Customizing the display label
            }));

            // 3. Add and refresh
            customerSelect.addOptions(options);
...
FILTER = {
    load() {
        // CSS for filter styles is now in manual.css
        FILTER.open();
        FILTER.filteredColumn();
        //FILTER.handleCheckboxFilters();
    },
    open() {
            ...
            if ($panel.is(':visible')) {
                ...
                $panel
                    ...
                    .slideDown(250, function () {
                        // Add arrow only once
                        if (!$panel.find('.filter-arrow').length) {
                            $panel.prepend('<div class="filter-arrow"></div>');
                        }
                    });
            }
        });
        ...
        $('#presetDateRange').on('change', function () {
            FILTER.setPresetDateRange();
        });
        ...
        $('#resetFilters').on('click', function () {
            ...
            if (typeof $.fn.tomSelect !== 'undefined') {
                if ($('#pol')[0].tomSelect) {
                    $('#pol')[0].tomSelect.clear();
                }
                if ($('#pod')[0].tomSelect) {
                    $('#pod')[0].tomSelect.clear();
                }
            }

            // Reset checkboxes
            $('#showCompleted, #showPending, #showCancelled, #showInvoiced').prop('checked', false);

            // Clear any active DataTable filters
            if ($.fn.DataTable.isDataTable('#dataTable')) {
    ...
    filteredColumn() {
        let formFilter = $('#list-filter'),
            defaultFilter = formFilter.find('[type=date].default-filter,[type=text].default-filter,[type=checkbox].default-filter,[type=radio]:checked.default-filter,select.default-filter').not('.avoid-filter'),
            manualFilter = formFilter.find('input:not([type=hidden],[type=radio]:not(:checked)),select:not([id=periods])').not('.avoid-filter,.default-filter'),
            filteredLabel = '', labelText = '',
            filters = $('#filters'),
            singleVal, e, eType, e2, multipleVal, removeIcon, filterInputArray, filterInputs;
        filterInputArray = [defaultFilter, manualFilter];
        let filteredData = $('#filtered-data');
        for (let m = 0; m <= 1; m++) {
            filterInputs = filterInputArray[m];
            for (let i = 0; i < filterInputs.length; i++) {
                ...
                e = filterInputs.eq(i);
                id = e.attr('id');
                eType = e.prop('nodeName');
                singleVal = e.val();
                labelText = 'first';
                if (singleVal && singleVal.length > 0) {
                    if (eType === 'SELECT') {
                        if ((singleVal.toString().split(',')).length > 1) {
                            multipleVal = '<b>' + singleVal.length + '</b>' + '&nbsp;' + e.closest('.form-filter').find('label').text();
                        } else {
                            singleVal = e.find(":selected").text();
                        }
                    } else if (e.attr('type') === 'checkbox') {
                        singleVal = e.is(':checked') ? 'Yes' : 'No';
                    } else if (e.attr('type') === 'radio') {
                        singleVal = e.data('label');
                        labelText = 'last';
                    }
        ...
        if (filteredLabel) {
            (filteredData) ? filteredData.html(filteredLabel) : filters.html('<div class="d-flex flex-wrap align-items-center" id="filtered-data">' + filteredLabel + '</div>');
        }
        this.removeFilter();
    },
    removeFilter() {
        $('.remove-label').off().on({
            click() {
                ...
                // Clear normal input/select
                removeId.val('');

                // Clear TomSelect properly
                if (removeId[0] && removeId[0].tomselect) {
                    removeId[0].tomselect.clear();          // clears selected value
    ...
    setPresetDateRange() {
        const select = document.getElementById('presetDateRange');
    ...
    handleCheckboxFilters() {
        ...
        $('#showCompleted, #showPending, #showCancelled, #showInvoiced').on('change', function () {
            // You can add custom filtering logic here
            // For example, update the DataTable based on checkbox states

            // Get checkbox states
            const showCompleted = $('#showCompleted').is(':checked');
            const showPending = $('#showPending').is(':checked');
            const showCancelled = $('#showCancelled').is(':checked');
            const showInvoiced = $('#showInvoiced').is(':checked');
            ...
            if ($.fn.DataTable.isDataTable('#dataTable')) {
                ...
                $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
                    // Get status from the row data (adjust column index as needed)
                    const status = data[2]; // Assuming status is in the 3rd column

                    // Filter based on checkbox states
                    if (showCompleted && status.includes('Completed')) return true;
                    if (showPending && status.includes('Pending')) return true;
                    if (showCancelled && status.includes('Cancelled')) return true;
                    if (showInvoiced && status.includes('Invoiced')) return true;

                    // If no checkboxes are checked, show all rows
                    return !showCompleted && !showPending && !showCancelled && !showInvoiced;
                });
...
const INPUT = {
    ...
    sanitizeValue(el) {
        ...
        // Use el.dataset.decimal (native) instead of el.attr
        const decimalDigits = el.dataset.decimal ?? 2; // Default to 2 if not set
        ...
        if (isInteger) {
            val = val.replace(/\D/g, ''); // Remove everything except digits
        }

        if (isFloat) {
            // 1. Remove everything except numbers and dots
            // 2. Remove any dots after the first one
            val = val
                .replace(/[^0-9.]/g, '')
                .replace(/(\..*)\./g, '$1');

            // 3. Dynamic Regex for decimal limit
            // We use double backslashes \\d and \\. because it is a string
            const pattern = `^(\\d+)(\\.\\d{0,${decimalDigits}})?.*$`;
    ...
    ensureDefaultZero(el) {
        ...
        if (!val || val === '-' || val === '.' || val === '-.') {
            ...
            if (el.classList.contains('float')) {
                el.value = '0.' + '0'.repeat(el.dataset.decimal ?? INPUT.decimalDigits);
            } else {
                el.value = '0';
            }
        }
